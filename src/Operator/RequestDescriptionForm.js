        ...
        <div className="request-description-form">
          ...
          <form>
            ...
            <div className="form-group">
              ...
              <textarea id="dialog" name="dialog" value={request.dialog} readOnly className="auto-height-textarea" />
            </div>
            {renderButtons()}
            <div>
              {dataArray.length > 0 ? (
                dataArray.map((med) => (
                  <div className="request-item" key={med.id}>
                    <div className="request-id">{med.id}</div>
                    <div className="request-subject">{med.UserRequestId}</div>
                    <div>
                      <button type="button" onClick={() => handleShowPhoto(med.id)}>
                        Показать фото
                      </button>
                    </div>
                  </div>
